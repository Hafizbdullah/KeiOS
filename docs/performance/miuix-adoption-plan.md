...
| M8 | Landed | Three sub-items: (a) extracted shared `BindScrollToTopEffect` in `widget/chrome/`, adopted by MCP, GitHub, OS; (b) extracted generic `TabbedPageBottomChrome<C : TabbedPageCategory>` in `widget/chrome/` — Settings/About bottom chrome were near-clones (313 vs 319 lines, 100% structurally identical), now thin wrappers; (c) audited backdrop activation convergence — `rememberMainPageBackdropSet` creates 3 backdrops (topBar, content, sheet) for action-bar pages, but Settings/About use 2 backdrops (topBar, bottomBar) with different semantics, so forced migration would be overengineering. BA Student Guide uses custom draw blocks. No code change for backdrop activation. |
| M9 | Verified | Re-verified current MIUIX snapshot `0.9.2-94c98f3e-SNAPSHOT`. The earlier consumed `miuix-blur` audit still applies: adaptive cascade downsampling (1/2/4/8/16 via `computeDownScaleParams`) + separable H/V Gaussian (`BlurEffect.kt`); compile-once shader pooling (`RuntimeShaderCacheImpl.obtainRuntimeShader` getOrPut, GC'd on dispose); GraphicsLayer pooling (`createGraphicsLayer`/`releaseGraphicsLayer` on detach, separate `noiseLayer` for dithering in `DrawBackdropModifier.kt`); zero-alloc scratch buffers. The latest upstream commits mainly improve squircle SDF upload/filtering, squircle clip/surface scroll performance, tall-content transparency, and spring-based indication animations. KeiOS gate `appGlassRuntimeEffectsEnabled()` delegates to `isRuntimeShaderSupported()`; `activeGlassBackdrop()` nulls the backdrop when unsupported. No direct blur/backdrop rewrite needed on this bump. |
...
- 2026-05-30 M8 page utility convergence first pass (scroll-to-top):
  ...
  - `git diff --check`
  - `rg "collectAsState\\(" app/src/main/java/os/kei -g '*.kt'`
- 2026-05-30 M8c bottom bar chrome convergence:
  - Audited Settings vs About bottom chrome: 313 vs 319 lines, 100%
    structurally identical (only category type and label prefixes differ).
  - Created `TabbedPageCategory` interface (`iconRes`, `labelRes`) in
    `widget/chrome/TabbedPageBottomChrome.kt`.
  - Made `SettingsCategory` and `AboutCategory` implement the interface.
  - Extracted generic `TabbedPageBottomChrome<C : TabbedPageCategory>` with
    shared constants (`TabbedPageBottomChromeSearchGap` etc.) and utility
    functions (`tabbedPageExpandedSearchWidth`, `tabbedPageCollapsedDockWidth`).
  - Replaced `SettingsBottomChrome` and `AboutBottomChrome` with thin wrappers
    (45 lines each, down from 313/319).
  - Updated `AboutBottomChromeLayoutTest` to use shared function names.
  - `./gradlew :app:compileDebugKotlin`
  - `./gradlew :app:testDebugUnitTest`
  - `git diff --check`
  - `rg "collectAsState\\(" app/src/main/java/os/kei -g '*.kt'`
- 2026-05-30 M8b backdrop activation convergence:
  - Audited `rememberMainPageBackdropSet` adoption feasibility for Settings,
    About, and BA Student Guide.
  - The helper creates 3 backdrops (topBar, content, sheet) for action-bar
    pages (GitHub, MCP, OS). Settings/About use 2 backdrops (topBar, bottomBar)
