...
internal fun LazyListScope.renderGuideVoiceTabContent(
    ...
    val structuredVoiceEntries =
        guide.voiceEntries.filter { entry ->
            entry.lines.any { line -> line.trim().isNotBlank() }
        }
    val migratedVoiceEntries =
        buildGrowthTitleVoiceEntries(
            guide
                .profileRowsForDisplay()
                .filter(::isGrowthTitleVoiceRow),
        )
    val voiceEntries =
        (structuredVoiceEntries + migratedVoiceEntries)
            .distinctBy { entry ->
                listOf(
                    entry.section.trim(),
                    entry.title.trim(),
    ...
    val voiceLanguageHeaders =
        buildVoiceLanguageHeadersForDisplay(
            headers = guide.voiceLanguageHeaders,
            entries = voiceEntries,
        )
    val dubbingHeaders =
        buildDubbingHeadersForVoiceCard(
            headers = voiceLanguageHeaders,
            entries = voiceEntries,
        )
    val selectedDubbingHeader =
        dubbingHeaders.firstOrNull { header ->
    ...
    if (voiceCvByLanguage.isNotEmpty()) {
        item(
            key = "guide-voice-cv",
            contentType = GuideVoiceContentType.CV,
        ) {
            GuideLiquidCard(
        ...
        item(
            key = "guide-voice-cv-spacer",
            contentType = GuideVoiceContentType.SPACER,
        ) { Spacer(modifier = Modifier.height(10.dp)) }
    }

    if (voiceEntries.isNotEmpty() && dubbingHeaders.isNotEmpty()) {
        item(
            key = "guide-voice-language",
            contentType = GuideVoiceContentType.LANGUAGE,
        ) {
        ...
        item(
            key = "guide-voice-language-spacer",
            contentType = GuideVoiceContentType.SPACER,
        ) { Spacer(modifier = Modifier.height(10.dp)) }
    }
    ...
    if (voiceEntries.isNotEmpty()) {
        voiceEntries.forEachIndexed { index, entry ->
            ...
            item(
                key = guideVoiceEntryKey(entry, index),
                contentType = GuideVoiceContentType.ENTRY,
            ) {
                val normalizedPlaybackUrl =
                    remember(context, sourceUrl, directPlaybackUrl, galleryCacheRevision) {
                        normalizeGuidePlaybackSource(
                ...
                val isCurrentPlayback =
                    normalizedPlaybackUrl.isNotBlank() &&
                        isVoicePlaying &&
            ...
            if (index < voiceEntries.lastIndex) {
                item(
                    key = "guide-voice-entry-spacer-$index",
                    contentType = GuideVoiceContentType.SPACER,
                ) { Spacer(modifier = Modifier.height(10.dp)) }
            }
        }
        ...
        item(
            key = "guide-voice-empty",
            contentType = GuideVoiceContentType.EMPTY,
        ) {
            GuideLiquidCard(
                modifier = Modifier.fillMaxWidth(),
                surfaceColor = Color(0x223B82F6),
...
private object GuideVoiceContentType {
    const val CV = "guide_voice_cv"
    const val LANGUAGE = "guide_voice_language"
    const val ERROR = "guide_voice_error"
    const val ENTRY = "guide_voice_entry"
    const val EMPTY = "guide_voice_empty"
    const val SPACER = "guide_voice_spacer"
}

private fun guideVoiceEntryKey(
    entry: BaGuideVoiceEntry,
    index: Int,
): String =
    buildString {
        append("guide-voice-entry-")
        append(entry.section.trim())
        append('|')
        append(entry.title.trim())
        append('|')
        append(entry.audioUrl.trim())
        append('|')
        append(entry.audioUrls.joinToString("|") { it.trim() })
        append('|')
        append(index)
    }
