...
@Composable
internal fun KeiOSJsonImportContent(
    ...
    AppPageLazyColumn(
        ...
        item(key = "status", contentType = "json_import_status") {
            JsonImportStatusCard(state)
        }
        item(key = "source", contentType = "json_import_source") {
            JsonImportSourceCard(state)
        }
        state.preview?.let { preview ->
            item(key = "preview", contentType = "json_import_preview") {
                JsonImportPreviewCard(preview)
            }
            if (preview.samples.isNotEmpty()) {
                item(key = "samples", contentType = "json_import_samples") {
                    JsonImportSamplesCard(preview.samples)
                }
            }
        }
        item(key = "actions", contentType = "json_import_actions") {
    ...
    AppFeatureCard(
        ...
        subtitle =
            pluralStringResource(
                R.plurals.json_import_samples_summary,
                samples.size,
                samples.size,
            ),
