...
import os.kei.feature.github.data.remote.GitHubReleaseAssetFetchSources
...
private val armSixtyFourTokenRegex = Regex("""(?:^|[^a-z0-9])arm64(?:[^a-z0-9]|$)""")
private val armeabiTokenRegex = Regex("""(?:^|[^a-z0-9])armeabi(?:[^a-z0-9]|$)""")
private val xEightySixTokenRegex = Regex("""(?:^|[^a-z0-9])x86(?:[^a-z0-9]|$)""")
...
internal fun VersionCheckUi.apkAssetTarget(
    ...
    if (alwaysLatestRelease || allowLatestReleaseFallback) {
        ...
        val latestByDate =
            listOf(stableCandidate, preCandidate)
    ...
    return when {
        recommendsPreRelease && preTag.isNotBlank() -> {
            ApkAssetTarget(
                rawTag = preTag,
        ...
        hasUpdate == true && stableTag.isNotBlank() -> {
            ApkAssetTarget(
                rawTag = stableTag,
        ...
        hasPreReleaseUpdate && preTag.isNotBlank() -> {
            ApkAssetTarget(
                rawTag = preTag,
                releaseUrl = latestPreUrl.ifBlank { GitHubVersionUtils.buildReleaseTagUrl(owner, repo, preTag) },
                label = context.getString(R.string.github_asset_target_prerelease),
...
internal fun prefersApiAssetTransport(asset: GitHubReleaseAssetFile): Boolean = asset.apiAssetUrl.isNotBlank()

internal fun assetTransportLabel(
    asset: GitHubReleaseAssetFile,
    context: Context,
): String = if (prefersApiAssetTransport(asset)) "API" else context.getString(R.string.github_asset_transport_direct)

internal fun bundleTransportLabel(
...
internal fun bundleCommitLabel(bundle: GitHubReleaseAssetBundle?): String? = bundle?.shortCommitSha?.trim().takeIf { !it.isNullOrBlank() }
...
internal fun assetAbiLabel(fileName: String): String? {
    ...
    return when {
        "arm64-v8a" in lower || "aarch64" in lower ||
            armSixtyFourTokenRegex.containsMatchIn(lower) -> "arm64"
        ...
        xEightySixTokenRegex.containsMatchIn(lower) -> "x86"
...
internal fun assetRelativeTimeLabel(
    ...
    val diffMillis = (nowMillis - updatedAt).coerceAtLeast(0L)
...
internal fun assetIsPreferredForDevice(
    ...
    val deviceAbiSet =
        supportedAbis
            ...
            .toSet()
    val hasArmSixtyFour = deviceAbiSet.any { abi -> "arm64" in abi || "aarch64" in abi }
    val hasArmThirtyTwo =
        deviceAbiSet.any { abi ->
            "armeabi" in abi || "armv7" in abi || abi.startsWith("arm")
        }
    val hasXEightySixSixtyFour = deviceAbiSet.any { abi -> "x86_64" in abi }
    val hasXEightySix =
        deviceAbiSet.any { abi ->
            xEightySixTokenRegex.containsMatchIn(abi)
        }
    return when (assetAbi) {
        "arm64" -> hasArmSixtyFour
        "armeabi-v7a", "armeabi" -> !hasArmSixtyFour && hasArmThirtyTwo
        "x86_64" -> hasXEightySixSixtyFour
        "x86" -> !hasXEightySixSixtyFour && hasXEightySix
        else -> false
    }
}
