...
internal enum class StarImportViewFilter(
...
internal enum class StarImportConflictStrategy(
    @param:StringRes val labelRes: Int,
) {
...
internal fun defaultVisibleStarImportQualities(): Set<GitHubStarImportQuality> =
    setOf(GitHubStarImportQuality.LikelyAndroid, GitHubStarImportQuality.NeedsReview)
...
internal fun GitHubRepositoryImportCandidate.matchesStarImportQuality(quality: GitHubStarImportQuality): Boolean =
    GitHubStarImportClassifier.classify(this) == quality
...
internal data class StarImportCandidateListUiState(
    ...
    val filteredRows: List<StarImportCandidateRowUiState>,
...
@Immutable
internal data class StarImportCandidateRowUiState(
    val candidate: GitHubRepositoryImportCandidate,
    val selected: Boolean,
    val trackedSelectable: Boolean,
    val apkVerificationState: StarImportApkVerificationUiState?,
) {
    val id: String = candidate.trackedApp.id
}
...
internal fun buildStarImportCandidateListUiState(
    ...
    val filteredRows = ArrayList<StarImportCandidateRowUiState>()
    ...
    candidates.forEach { candidate ->
        ...
        val selected = selectable && id in selectedIds
        if (selected) {
        ...
        filteredRows +=
            StarImportCandidateRowUiState(
                candidate = candidate,
                selected = selected,
                trackedSelectable = trackedSelectable,
                apkVerificationState = verificationState,
            )
        if (selectable) {
    ...
    val qualityFilterCounts =
        GitHubStarImportQuality.entries.associateWith { quality ->
...
private fun GitHubRepositoryImportCandidate.matchesStarImportQuery(query: String): Boolean =
    repository.fullName.contains(query, ignoreCase = true) ||
...
internal fun String.isValidGitHubUsernameInput(): Boolean = toGitHubUsernameInput().isNotBlank()
...
internal fun String.toGitHubStarsRepositoryUrlInput(): String {
    ...
    return if (parts.size == 1 &&
        parsed.query.orEmpty().split('&').any {
...
private fun String.gitHubStarsInputPath(): String? {
    ...
    return when {
        ...
        raw.startsWith("/stars/", ignoreCase = true) -> {
            raw.substringBefore('?')
        }
...
private fun String.trimTrailingZeroDecimal(): String = replace(".0", "")

private val githubUsernameInputRegex = Regex("""[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?""")

private val reservedGitHubProfileSegments =
    setOf(
        "about",
        "apps",
        "blog",
