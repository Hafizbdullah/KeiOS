...
internal object McpToolCatalogDescriptions {
    val english = mapOf(
        ...
        "keios.app.info" to "Read app identity, version, and disabled, Shizuku, or Root privilege state.",
        "keios.app.version" to "Read versionName and versionCode.",
        "keios.privilege.status" to "Read the selected disabled, Shizuku, or Root privilege backend and command readiness.",
        ...
        "keios.home.overview.snapshot" to "Read Home overview data for MCP, GitHub, Blue Archive, and WebDAV.",
        ...
        "keios.github.tracks.list" to "List tracked GitHub repository, Git repository, subscription, or F-Droid entries. Args: repoFilter, sourceMode=github_repository|git_repository|direct_apk|fdroid_repository, filterMode=all|github_repository|git_repository|direct_apk|fdroid_repository|pre_release_tracked|update_available|installed|failed_checks|actions_check_enabled, sortMode=update|name|pre_release|changed|added, sortDirection=forward|reverse, limit.",
        "keios.github.tracks.export" to "Export tracked GitHub repository, Git repository, subscription, or F-Droid entries as keios.github.tracked/v4 JSON. Args: repoFilter, sourceMode, filterMode, sortMode, sortDirection.",
        "keios.github.tracks.import" to "Preview or import tracked GitHub repository, Git repository, subscription, and F-Droid JSON. Args: json, apply.",
        "keios.github.tracks.check" to "Check tracked GitHub repository, Git repository, subscription, or F-Droid entries online. Args: repoFilter, sourceMode, filterMode, sortMode, sortDirection, onlyUpdates, limit.",
        ...
        "keios.github.refresh.history" to "Read local GitHub refresh history for troubleshooting. Args: mode=summary|list|detail, outcome, source, scope, sinceMillis, includeFailures, limit.",
        "keios.github.refresh.history.export" to "Export local GitHub refresh history as JSON for diagnostics. Args: outcome, source, scope, sinceMillis, limit.",
        ...
        "keios.ba.snapshot" to "Read one Blue Archive account's AP, Cafe, cooldown, Craft Chamber, notification, server, and ID snapshot. Args: accountId (empty = active).",
        "keios.ba.accounts" to "List every Blue Archive account with its own AP, Cafe, cooldowns, Craft Chamber state, reminder mode, and daily-done tile binding.",
        "keios.ba.daily.done" to "Apply the dailies template: zero both AP pools, restart elapsed Cafe cooldowns, load free Craft slots 1-2. Args: accountId (empty = all enabled), apply (false previews).",
        ...
        "keios.webdav.status" to "Read credential-safe WebDAV configuration, auto-sync, item, pending-review, and remote-probe status.",
        "keios.webdav.history" to "Read credential-safe WebDAV sync diagnostics. Args: mode=summary|list|detail, limit, issuesOnly, id.",
    ...
    private val chinese = mapOf(
        ...
        "keios.app.info" to "读取应用身份、版本与关闭、Shizuku 或 Root 特权状态。",
        "keios.app.version" to "读取 versionName 与 versionCode。",
        "keios.privilege.status" to "读取已选择的关闭、Shizuku 或 Root 特权后端及命令就绪状态。",
        ...
        "keios.home.overview.snapshot" to "读取主页概览数据，包含 MCP、GitHub、Blue Archive 与 WebDAV。",
        ...
        "keios.github.tracks.list" to "列出已追踪 GitHub 仓库、Git 仓库、订阅项目或 F-Droid 来源。参数：repoFilter、sourceMode=github_repository|git_repository|direct_apk|fdroid_repository、filterMode=all|github_repository|git_repository|direct_apk|fdroid_repository|pre_release_tracked|update_available|installed|failed_checks|actions_check_enabled、sortMode=update|name|pre_release|changed|added、sortDirection=forward|reverse、limit。",
        "keios.github.tracks.export" to "将已追踪 GitHub 仓库、Git 仓库、订阅项目或 F-Droid 来源导出为 keios.github.tracked/v4 JSON。参数：repoFilter、sourceMode、filterMode、sortMode、sortDirection。",
        "keios.github.tracks.import" to "预览或导入已追踪 GitHub 仓库、Git 仓库、订阅项目与 F-Droid JSON。参数：json、apply。",
        "keios.github.tracks.check" to "联网检查已追踪 GitHub 仓库、Git 仓库、订阅项目或 F-Droid 来源更新。参数：repoFilter、sourceMode、filterMode、sortMode、sortDirection、onlyUpdates、limit。",
        ...
        "keios.github.refresh.history" to "读取本地 GitHub 刷新历史，方便排查刷新卡住、失败或耗时异常。参数：mode=summary|list|detail、outcome、source、scope、sinceMillis、includeFailures、limit。",
        "keios.github.refresh.history.export" to "将本地 GitHub 刷新历史导出为诊断 JSON。参数：outcome、source、scope、sinceMillis、limit。",
        ...
        "keios.ba.snapshot" to "读取单个 Blue Archive 账号的 AP、咖啡厅、冷却、制造工坊、通知、服务器与 ID 快照。参数：accountId（留空为当前账号）。",
        "keios.ba.accounts" to "列出全部 Blue Archive 账号，各自的 AP、咖啡厅、冷却、制造工坊状态、提醒模式与日常磁贴绑定。",
        "keios.ba.daily.done" to "套用日常模板：两个 AP 清零、重启已冷却完毕的咖啡厅计时、装填空闲的制造 1-2 槽。参数：accountId（留空为全部启用账号）、apply（false 仅预览）。",
        ...
        "keios.webdav.status" to "读取不含凭据的 WebDAV 配置、自动同步、同步项、待处理冲突与远端探测状态。",
        "keios.webdav.history" to "读取不含凭据的 WebDAV 同步诊断。参数：mode=summary|list|detail、limit、issuesOnly、id。",
    ...
    private val japanese = mapOf(
        ...
        "keios.app.info" to "アプリ情報、version、無効・Shizuku・Root の特権状態を読み取ります。",
        "keios.app.version" to "versionName と versionCode を読み取ります。",
        "keios.privilege.status" to "選択中の無効・Shizuku・Root 特権バックエンドとコマンド実行可否を読み取ります。",
        ...
        "keios.home.overview.snapshot" to "MCP、GitHub、Blue Archive、WebDAV を含む Home 概要データを読み取ります。",
        ...
        "keios.github.tracks.list" to "追跡中の GitHub リポジトリ、Git リポジトリ、購読プロジェクト、F-Droid ソースを一覧します。引数: repoFilter、sourceMode=github_repository|git_repository|direct_apk|fdroid_repository、filterMode=all|github_repository|git_repository|direct_apk|fdroid_repository|pre_release_tracked|update_available|installed|failed_checks|actions_check_enabled、sortMode=update|name|pre_release|changed|added、sortDirection=forward|reverse、limit。",
        "keios.github.tracks.export" to "追跡中の GitHub リポジトリ、Git リポジトリ、購読プロジェクト、F-Droid ソースを keios.github.tracked/v4 JSON としてエクスポートします。引数: repoFilter、sourceMode、filterMode、sortMode、sortDirection。",
        "keios.github.tracks.import" to "追跡中の GitHub リポジトリ、Git リポジトリ、購読プロジェクト、F-Droid JSON をプレビューまたはインポートします。引数: json、apply。",
        "keios.github.tracks.check" to "追跡中の GitHub リポジトリ、Git リポジトリ、購読プロジェクト、F-Droid ソースの更新をオンライン確認します。引数: repoFilter、sourceMode、filterMode、sortMode、sortDirection、onlyUpdates、limit。",
        ...
        "keios.github.refresh.history" to "ローカル GitHub 更新履歴を読み取り、更新の停止、失敗、時間超過の診断に使います。引数: mode=summary|list|detail、outcome、source、scope、sinceMillis、includeFailures、limit。",
        "keios.github.refresh.history.export" to "ローカル GitHub 更新履歴を診断 JSON としてエクスポートします。引数: outcome、source、scope、sinceMillis、limit。",
        ...
        "keios.ba.snapshot" to "Blue Archive アカウント 1 件の AP、カフェ、クールダウン、製造工房、通知、サーバー、ID スナップショットを読み取ります。引数: accountId（空欄で現在のアカウント）。",
        "keios.ba.accounts" to "すべての Blue Archive アカウントを、それぞれの AP、カフェ、クールダウン、製造工房の状態、リマインダーモード、デイリータイルの割り当てとともに一覧します。",
        "keios.ba.daily.done" to "デイリーテンプレートを適用します: AP を両方ゼロにし、クールダウン済みのカフェ計時を再開し、空いている製造 1-2 枠を装填します。引数: accountId（空欄で有効な全アカウント）、apply（false はプレビュー）。",
        ...
        "keios.webdav.status" to "認証情報を含まない WebDAV 設定、自動同期、同期項目、要確認状態、リモート確認状態を読み取ります。",
        "keios.webdav.history" to "認証情報を含まない WebDAV 同期診断を読み取ります。引数: mode=summary|list|detail、limit、issuesOnly、id。",
